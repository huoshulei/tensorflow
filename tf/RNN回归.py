import tensorflow as tf
import numpy as np
import matplotlib.pyplot as plt
from tensorflow.contrib.legacy_seq2seq.python.ops.seq2seq import sequence_loss_by_example

BATCH_START = 0
TIME_STEPS = 20
BATCH_SIZE = 50
INPUT_SIZE = 1
OUTPUT_SIZE = 1
CELL_SIZE = 10
LR = 0.005


def get_batch():
    """绘制数据曲线"""
    global BATCH_START, TIME_STEPS
    xs = np.arange(BATCH_START, BATCH_START + TIME_STEPS * BATCH_SIZE).reshape((BATCH_SIZE, TIME_STEPS)) / (np.pi * 10)
    seq = np.sin(xs)
    res = np.cos(xs)
    BATCH_START += TIME_STEPS
    return [seq[:, :, np.newaxis], res[:, :, np.newaxis], xs]


class LSTMRNN(object):
    def __init__(self, n_steps, input_size, output_size, cell_size, batch_size):
        self.n_steps = n_steps
        self.input_size = input_size
        self.output_size = output_size
        self.cell_size = cell_size
        self.batch_size = batch_size
        self.xs = tf.placeholder(tf.float32, [None, n_steps, input_size])
        self.ys = tf.placeholder(tf.float32, [None, n_steps, output_size])
        with tf.variable_scope('in_hidden'):
            self.l_in_y = self.add_input_layer()
        self.cell_outputs, self.cell_final_state = self.add_cell()
        with tf.variable_scope('out_hidden'):
            self.prediction = self.add_output_layer()
        self.compute_cost()
        self.train_ao = tf.train.AdamOptimizer(LR).minimize(self.cost)

    def add_input_layer(self):
        l_in_x = tf.reshape(self.xs, [-1, self.input_size])
        ws_in = self._weight_variable([self.input_size, self.cell_size])
        bs_in = self._bias_variable([self.cell_size])
        l_in_y = tf.matmul(l_in_x, ws_in) + bs_in
        return tf.reshape(l_in_y, [-1, self.n_steps, self.cell_size])

    def add_cell(self):
        # lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(self.cell_size)
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(self.cell_size)
        self.cell_init_state = lstm_cell.zero_state(self.batch_size, dtype=tf.float32)
        return tf.nn.dynamic_rnn(lstm_cell, self.l_in_y,
                                 initial_state=self.cell_init_state,
                                 time_major=False)

    def add_output_layer(self):
        l_out_x = tf.reshape(self.cell_outputs, [-1, self.cell_size])
        ws_out = self._weight_variable([self.cell_size, self.output_size])
        bs_out = self._bias_variable([self.output_size])
        return tf.matmul(l_out_x, ws_out) + bs_out

    def compute_cost(self):
        """from tensorflow.contrib.legacy_seq2seq.python.ops.seq2seq import sequence_loss_by_example"""
        losses = sequence_loss_by_example(
            [tf.reshape(self.prediction, [-1], name='reshape_pred')],
            [tf.reshape(self.ys, [-1], name='reshape_target')],
            [tf.ones([self.batch_size * self.n_steps], dtype=tf.float32)],
            average_across_timesteps=True,
            softmax_loss_function=self.ms_error,
            name='losses'
        )
        self.cost = tf.div(
            tf.reduce_sum(losses, name='losses_sum'),
            self.batch_size,
            name='average_cost')
        tf.summary.scalar('cost', self.cost)

    @staticmethod
    def ms_error(labels, logits):
        return tf.square(tf.subtract(labels, logits))

    def _weight_variable(self, shape):
        return tf.get_variable('weights', shape=shape, initializer=tf.random_normal_initializer(mean=0., stddev=1.))

    def _bias_variable(self, shape):
        return tf.get_variable('bias', shape=shape, initializer=tf.constant_initializer(0.1))


if __name__ == '__main__':
    model = LSTMRNN(TIME_STEPS, INPUT_SIZE, OUTPUT_SIZE, CELL_SIZE, BATCH_SIZE)
    with tf.Session() as sess:
        sess.run(tf.initialize_all_variables())
        plt.ion()
        plt.show()
        for i in range(333):
            seq, res, xs = get_batch()
            if i == 0:
                feed_dict = {
                    model.xs: seq,
                    model.ys: res
                }
            else:
                feed_dict = {
                    model.xs: seq,
                    model.ys: res,
                    model.cell_init_state: state
                }
            _, cost, state, prediction = sess.run(
                [model.train_ao, model.cost, model.cell_final_state, model.prediction], feed_dict=feed_dict)
            plt.plot(xs[0, :], res[0].flatten(), 'r', xs[0, :], prediction.flatten()[:TIME_STEPS], 'b--')
            plt.ylim((-1.2, 1.2))
            plt.draw()
            plt.pause(0.2)
